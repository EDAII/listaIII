...
int *somavet(int **result, int tam, int tam2)
{
  int i, ivet, icountb, itensbalde, counttot;

  ivet = 0;
  counttot = 0;
  for (i = 0; i < tam; i++)
  {
    itensbalde = result[i][0];
    printf("%d\n", itensbalde);
    counttot = counttot + result[i][0];
  }
  int v[counttot];
  for (i = 0; i < tam; i++)
  {
    itensbalde = result[i][0];
    for (icountb = 1; icountb <= itensbalde; icountb++)
    {
      v[ivet] = result[i][icountb];
      ivet++;
    }
  }
  for (ivet = 0; ivet < counttot; ivet++)
  {
    printf("o valor  %d do vetor e %d\n", ivet, v[ivet]);
  }
}
int main(int argc, char const *argv[])
{
 ...
 if (balde1 == 0)
  ...
  printf("%d", gap);
  int *v2;
  v2 = (int *)malloc((nbalde + 1) * sizeof(int));
  int i2, i3;

  for (i = 0; i < nbalde; i++)
  {
    for (i2 = 1; i2 <= nbalde; i2++)
    {
      v2[i2] = result[i][i2];
    }
    sheelsortg(gap, interbalde, v2);
    for (i3 = 0; i3 < interbalde; i3++)
    {
      printf(" numero %d ,%d\n", i3, v2[i3]);
    }
    for (i2 = 1; i2 < nbalde; i2++)
